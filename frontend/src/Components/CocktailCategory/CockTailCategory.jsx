        ...
        <section className="pt-[10%] px-8">
            ...
            <ul className='flex flex-col sm:flex-row gap-6 justify-center flex-wrap lg:w-1/2 mx-auto'>
                {cocktail?.map((category) => (
                    <Link key={category.strCategory} to={`/drink/category/${category.strCategory}`} className='px-6 py-3 bg-yellow-300 hover:bg-yellow-200 rounded-full font-semibold text-center'>{category.strCategory}</Link>
                ))}
            </ul>
        </section>
    )
